...
def validate_https_url(url: str, service_id: str) -> None:
    ...
    if (
        parts.scheme != "https"
        or not (
            hostname == "yonsei.ac.kr"
            or hostname.endswith(".yonsei.ac.kr")
        )
        ...
        raise CatalogError(
            f"Service {service_id!r} must use HTTPS port 443 on an official "
            "yonsei.ac.kr host."
        )
...
def probe(service_id: str, service: dict[str, Any], timeout: float) -> dict[str, Any]:
    ...
    if not is_allowed_redirect(effective_url):
        return {
            "schema": "yonsei-service-probe/v1",
            "service": service_id,
            "label": service["label"],
            ...
            "vpn_requirement": "not-established",
            "error": (
                "The effective response URL left the HTTPS Yonsei port-443 boundary."
            ),
        }
