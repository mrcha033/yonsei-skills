...
class LearnUsClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.session_factory = session_factory or HttpSession
    ...
    def fetch(self, url: str, *, max_bytes: int = DEFAULT_MAX_BYTES) -> FetchResult:
        ...
        with self.lock:
            ...
            if final_parts.hostname != urlsplit(self.origin).hostname:
                raise AuthorizationBoundaryError(
                    "LearnUs redirected the requested resource outside ys.learnus.org."
                )
...
def run_self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="learnus-headless-test-") as temporary:
        ...
        assert status_reply["authenticated"] is True
        checks += 1
    ...
    with tempfile.TemporaryDirectory(prefix="learnus-headless-daemon-test-") as temporary:
        ...
        assert daemon_status["authenticated"] is True
        checks += 1
