...
Checked 2026-07-27 with anonymous, read-only GET requests. No login form,
...
| Plugin | Verified evidence | Installable result scope | Still pending authenticated mapping | Disabled writes |
| --- | --- | --- | --- | --- |
| Academic | Official portal entry and documented student functions | supplied class, grade, and enrollment snapshot audits | current classes, grades, earned credits, graduation progress | profile, leave/return, registration, withdrawal, grade changes |
...
| Shuttle | Public program `P004023`; official client module and read/write controller names | entry diagnosis, supplied option filtering, supplied seat verdict | live trips, seats, waitlist, own reservation and receipt | reserve, waitlist, cancel, poll |
| Space | Public user guides and login roles | supplied availability filtering, public-rule checks, unsent request draft | current rooms, approval, payment requirement, own requests | request, cancel, pay, approve, permit issuance |
...
`AVAILABLE` means the packaged result above is implemented and tested. It does
not imply the pending authenticated adapter or a disabled write is available.
