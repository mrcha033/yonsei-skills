...
| Certificate | Official internet-certificate flow, locally pinned ReportX runtime assets, and authorized bundled YonseiB/YonseiL embedding checks | Windows native ReportX routing; macOS/Linux title/body-only font mapping, one-time document-number reservation, compatibility PDF review, and optional confirmed CUPS print | authenticated issuance on each OS with a student's chosen certificate and printer | certificate-field alteration, generic-font substitution, uncertain retry, paid electronic-certificate payment |
