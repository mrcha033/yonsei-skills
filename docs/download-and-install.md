...
macOS/Linux용 플러그인에는 재배포 허가를 받은 `연세제목.TTF`와
`연세본문.TTF`가 포함되어 있어 따로 설치할 필요가 없습니다. 제목과
본문을 서로 다른 PDF 글꼴로 넣으며, 파일 해시가 맞지 않으면 다른
글꼴로 대체하지 않고 발급 전에 중단합니다.
...
이 요청들은 내부적으로 다음 8개 빠른 기능을 사용합니다: 오늘의 연세,
학사신청·장학, 수강 마일리지, 졸업·교직, 셔틀, 공간·생활관, 증명서,
LearnUs·출결. 학생은 기능 이름이나 명령어를 외울 필요가 없습니다.
