...
브라우저 작업은 먼저 이미 로그인된 일반 Chrome·Edge·Chromium
프로필을 찾고, 없을 때만 Codex가 제어할 수 있는 기존 데스크톱
브라우저 프로필을 사용합니다. 학생이 별도 명령어를 실행할 필요는
없습니다. 공식 로그인 화면이 나타날 때만 그 화면에서 한 번 직접
로그인하면 Portal에서 Underwood로 이어서 진행합니다.
