...
실제 공식 서비스 실행과 로컬·3개 운영체제 검사를 구분한 결과는
[`docs/e2e-validation.md`](docs/e2e-validation.md)에서 확인할 수 있습니다.

셔틀·공간은 Windows, macOS, Linux에서 같은 공식 브라우저 흐름을
사용합니다. 무료 증명서 PDF도 세 운영체제 모두 학교의 인증된
인터넷즉시발급 화면과 번들된 로컬 PDF 프린터를 사용합니다. Windows에서
실물 프린터를 명시한 경우에만 공식 ReportX 인쇄 경로를 선택합니다.

`yonsei-student-companion`은 한 번 로그인한 전용 관리 브라우저와
학생용 공통 실행 도구를 제공합니다. 함께 설치한 수강·학사·셔틀·공간·
증명서·LearnUs 플러그인도 이 로그인을 이어 씁니다. 학생은 명령어나
포털 메뉴 위치를 외울 필요가 없습니다. 공간대관처럼 별도 로그인을
쓰는 서비스는 같은 관리 브라우저에서 최초 한 번 추가 인증합니다.
...
| `yonsei-course-registration` | 시간표와 마일리지 배분을 함께 짜고 싶을 때 | 충돌 없는 시간표와 개인 이력·현재 정원·신청자를 고려한 마일리지 전략 |
| `yonsei-attendance-copilot` | 결석·지각 현황을 확인하거나 정정 문의를 준비할 때 | 공식 전자출결을 읽기 전용으로 확인하고 문의 초안 작성, 출석 체크는 하지 않음 |
...
| [오늘의 연세](plugins/yonsei-student-companion/skills/summarize-yonsei-today/SKILL.md) `summarize-yonsei-today` | 포털 대시보드와 접근 가능한 Underwood 수업·장학·마일리지·졸업·교직, LearnUs·출결을 읽기 전용으로 모아 우선순위와 다음 행동을 보여 줍니다. | “오늘 수업이랑 LearnUs, 출결, 학사 진행 상황을 한 번에 정리해 줘.” |
...
| 스킬 | 상세 기능 | 이렇게 요청해 보세요 |
| --- | --- | --- |
| [공식 수강편람 조회·과목표 정리](plugins/yonsei-course-registration/skills/normalize-yonsei-courses/SKILL.md) `normalize-yonsei-courses` | 전용 브라우저의 포털 로그인을 이어 Underwood 수강편람을 직접 조회해 과목번호·분반·학점·교수·시간·캠퍼스로 정리합니다. 수강신청 기간이 아니어도 조회하며, 캡처·PDF·엑셀은 공식 조회 실패 시 보조 입력으로만 씁니다. | “2026년 2학기 공과대학 자료구조 과목을 찾아 시간표용으로 정리해 줘.” |
...
| [내부 복구: 수강 사이트 접속 진단](plugins/yonsei-course-registration/skills/diagnose-yonsei-course-access/SKILL.md) `diagnose-yonsei-course-access` | 정상 수강편람 조회가 실패했을 때만 연중 사용하는 인증된 Underwood 편람과 기간 중에 열리는 수강신청 화면을 분리해 점검합니다. 학생이 따로 실행할 필요는 없습니다. | “수강신청은 닫혔어도 수강편람에서 과목은 직접 찾아 줘.” |
...
| 스킬 | 상세 기능 | 이렇게 요청해 보세요 |
| --- | --- | --- |
| [셔틀 후보 정리](plugins/yonsei-shuttle-booking/skills/list-yonsei-shuttle-options/SKILL.md) `list-yonsei-shuttle-options` | 공식 셔틀 화면을 먼저 열어 날짜, 출발 캠퍼스, 시간 범위, 최소 잔여석에 맞는 편을 시간순으로 정리합니다. 공식 조회가 실패한 경우에만 캡처나 표를 보조 입력으로 씁니다. | “내일 오전 9시 전 신촌 출발 셔틀만 보여 줘.” |
...
| 스킬 | 상세 기능 | 이렇게 요청해 보세요 |
| --- | --- | --- |
| [조건에 맞는 공간 찾기](plugins/yonsei-space-reservation/skills/search-yonsei-spaces/SKILL.md) `search-yonsei-spaces` | 공식 공간 시스템을 먼저 열어 날짜·시간 전체 포함 여부, 최소 수용인원, 건물, 프로젝터 등 필수 장비로 후보를 거릅니다. 공식 조회가 실패한 경우에만 첨부 목록을 보조 입력으로 씁니다. | “다음 주 수요일 15명, 프로젝터 있는 스터디 공간 찾아 줘.” |
...
Windows·macOS·Linux 모두 학교의 무료 인터넷즉시발급 결과를 번들된
로컬 PDF 프린터로 저장합니다. 연세 제목체·본문체는 둘 다 검증하며,
실제 문서가 참조한 글꼴만 PDF에 포함합니다. 별도 유료 전자증명서
상품과 무료 출력 PDF의 차이를 숨기지 않습니다.
...
- 포털·LearnUs·셔틀·증명서는 전용 관리 브라우저 프로필을 재사용합니다.
  공간대관처럼 별도 로그인을 쓰는 서비스는 같은 창에서 최초 한 번
  추가 인증하고, 세션이 만료될 때만 다시 연결합니다.
...
> 내일 오전 9시 전에 신촌에서 국제캠퍼스로 가는 셔틀을 찾아 줘.

> 내 성적표와 2024학번 전공 졸업요건을 비교해서 부족한 과목을 계산해 줘.

> 내 마일리지 이력과 현재 정원을 고려해서 72점을 전략적으로 나눠 줘.

## 학생용 플러그인

수강 개인 이력·출결·셔틀·공간·학사·증명서·LearnUs의 라이브 브라우저
기능을 쓰려면 `yonsei-student-companion`을 먼저 한 번 설치하세요.
터미널 없는 Codex용 ZIP 안내도 이 순서로 되어 있습니다. 공지 검색과
첨부 자료만 분석하는 기능은 공통 브라우저 없이도 사용할 수 있습니다.

| 플러그인 | 설치 명령 | 할 수 있는 일 |
| --- | --- | --- |
| 포털·오늘의 연세 | `codex plugin add yonsei-student-companion@yonsei-skills` | 전용 관리 브라우저 로그인 재사용, 오늘 요약, 생활관·장학금·교환학생 관리 |
...
| 증명서 | `codex plugin add yonsei-certificate-assistant@yonsei-skills` | 증명서·학생활동·교육실습 문서를 Windows·macOS·Linux에서 무료 출력 PDF로 발급하고, Windows 실물 인쇄만 공식 ReportX로 처리 |
| LearnUs | `codex plugin add learnus-course-copilot@yonsei-skills` | 강의·마감일·자료 정리, 만료 시 공식 로그인 화면에서 다시 연결 |
...
- 플러그인은 출석 체크, 수강신청, 승인, 결제, 메시지 발송을 수행하지
  않습니다. 셔틀 예약·공간 신청·무료 프린트 증명서 발급만 정확한
  대상과 전송 내용을 보여 준 뒤 학생이 확인한 경우에 한 번 실행합니다.
...
- 세 운영체제의 무료 출력 PDF는 학교의 인증된 발급 응답을 로컬에서
  렌더링한 결과이며, 별도 유료 상품인 전자서명 전자증명서라는 뜻은
  아닙니다. Windows에서 실물 프린터를 요청한 경우에만 공식 ReportX를
  사용합니다.
- 재배포 허가를 받은 연세 제목체와 본문체가 플러그인에 포함되어 별도
  설치가 필요 없습니다. 두 파일의 무결성을 모두 검사하고, 실제 문서가
  사용한 글꼴만 PDF에 포함합니다. 다른 한글 글꼴이 발견되면 발급 전에
  중단합니다.
...
Codex는 마켓플레이스와 설치된 플러그인을 함께 갱신합니다.

```bash
codex plugin marketplace upgrade yonsei-skills
```

Claude Code는 마켓플레이스를 갱신한 뒤 설치한 플러그인도 각각
업데이트해야 합니다. 예:

```bash
claude plugin marketplace update yonsei-skills
claude plugin update yonsei-student-companion@yonsei-skills
claude plugin update yonsei-shuttle-booking@yonsei-skills
```
...
python3 -B scripts/run_plugin_tests.py
```
